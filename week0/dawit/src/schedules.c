...
// An illustration of several features of the C programming language.
...
// that descibes a process ( a task)
...
int main (int argc, char **argv){
	...
	//with the time measured in seconds.log
	// time_t is  just nother name for a long (64 bit) integer
	time_t t = time(NULL);
	srand(t);
...
// Must declare counter before beggining loop!
int i;
for( i = 0; i < 12; i++){
	//malloc() is the function for "memory allocation"
	//sizeof is an operatior that tells us how many bytes we will need to store an object of a given type.
	...
	cp->id =i;
	// draw a random number form an exponential distribuuition with a specified mean
	double r = ((double) rand())/RAND_MAX;
	cp->timeToService = -MEAN_SERVICE_TIME * log(r);
	// draw a random number from an exponential distribution with a specified mean 
...
//and end with the first elemenet we added to the list
cp = rootPointer;
while(cp != NULL) {
	printf("process id = %d\n", cp->id );
